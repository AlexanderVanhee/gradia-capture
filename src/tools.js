...
export const TOOLS = [
	...
	},
	{
		...
		bounds: makeStrokeBounds(
			(s) => SELECTION_PADDING + (s.strokeWidth ?? 4),
		),
	...
	},
	{
		...
		beginStroke: () => ({}),
		bounds: makeStrokeBounds(
			(s) => SELECTION_PADDING + (s.strokeWidth ?? 4) * 4,
		),
		hitTest: standardHitTest,
		render(cr, stroke, lineWidth) {
			if (stroke.points.length < 2) return;
	...
	},
	{
		...
		beginStroke: () => ({}),
		bounds: makeStrokeBounds(
			(s) => SELECTION_PADDING + (s.strokeWidth ?? 4) * 2,
		),
		hitTest: standardHitTest,
		render(cr, stroke, lineWidth) {
			if (stroke.points.length < 2) return;
			...
			cr.moveTo(p1.x, p1.y);
			cr.lineTo(
				p1.x - size * Math.cos(angle - spread),
				p1.y - size * Math.sin(angle - spread),
			);
			cr.moveTo(p1.x, p1.y);
			cr.lineTo(
				p1.x - size * Math.cos(angle + spread),
	...
	},
	{
		...
		bounds(stroke) {
			...
			const fontSize = Math.max(
				8,
				Math.round((stroke.strokeWidth ?? 4) * 3),
			);
	...
	},
	{
		...
		render(cr, stroke, lineWidth) {
			...
			const textColor =
				lum > 0.5 ? { r: 0, g: 0, b: 0 } : { r: 1, g: 1, b: 1 };
			...
			cr.moveTo(
				pt.x - extents.width / 2 - extents.x,
				pt.y - extents.height / 2 - extents.y,
			);

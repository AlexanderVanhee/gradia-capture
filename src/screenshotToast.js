...
class ScreenshotToast {
	constructor(file, imageContent, imgW, imgH, showCopied = false) {
		...
		if (file && isGradiaFlatpakInstalled()) {
			...
			this._signalIds.push([
				...
				this._editButton.connect("realize", () => {
					this._editButton.set_size(
						CLOSE_BUTTON_SIZE,
						CLOSE_BUTTON_SIZE,
					);
					this._editButton.set_position(
						btnMargin,
		...
		if (file) {
			...
			this._signalIds.push([
				...
				this._openFolderButton.connect("realize", () => {
					this._openFolderButton.set_size(
						CLOSE_BUTTON_SIZE,
						CLOSE_BUTTON_SIZE,
					);
		...
		if (showCopied) {
			...
			this._signalIds.push([
				...
				this._copiedLabel.connect("realize", () => {
					const [, naturalW] =
						this._copiedLabel.get_preferred_width(-1);
					const [, naturalH] =
						this._copiedLabel.get_preferred_height(-1);
		...
		this._closeButton.set_position(
			TOAST_WIDTH - CLOSE_BUTTON_OFFSET + 1,
			-1,
		);
		this._signalIds.push([
			this._closeButton,
...
export function showScreenshotToast(
	...
	_activeToast = new ScreenshotToast(
		file,
		imageContent,
		imgW,
		imgH,
